...
export default function Register() {
  ...
  // Merge client-side + server-side errors
  const allErrors = { ...errors, ...form.errors }

  const Field = ({ id, label, icon, type = 'text', placeholder, field, autoFocus }) => {
  ...
  return (
    <GuestLayout title="Create Account" description="Start your journey to smarter finances.">
      ...
      <form onSubmit={submit} className="space-y-4" noValidate>
        <Field id="name" label="Full Name" icon="person" placeholder="Nama Lengkap" field="name" autoFocus />
        <Field id="email" label="Email Address" icon="mail" type="email" placeholder="nama@email.com" field="email" />
        <Field id="password" label="Password" icon="lock" type="password" placeholder="••••••••" field="password" />
        <Field id="password_confirmation" label="Confirm Password" icon="lock" type="password" placeholder="••••••••" field="password_confirmation" />
